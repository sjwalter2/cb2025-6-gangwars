...
/// @param text1 - Array of Strings to display
...
function scr_draw_tooltip_ext(text1, world_x, world_y, exclude_tiles = undefined, text_color = c_white) {
	...
	var box_width = 0
	for (var _i = 0 ; _i < array_length(text1) ; _i += 1)
	{
		box_width = max(box_width,string_width(text1[_i]))
	}

    box_width = box_width + padding * 2;
    var box_height = string_height(text1[0]) * array_length(text1) + spacing*2 + padding * 2;
	...
	for (var j = 0 ; j < array_length(text1); j += 1 ){
		draw_text(best_x + box_width / 2, best_y + j*(padding + string_height(text1[0])) + spacing, text1[j]);
